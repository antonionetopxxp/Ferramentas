...
- **Cotação do dólar em tempo real** (API gratuita `open.er-api.com`, sem chave, com cache local de 6h)
- **Segurança**: CSP, headers de segurança no `_headers` (Netlify), `robots.txt`, `security.txt`, links com `rel="noopener noreferrer"`, `localStorage` validado
- **Animações**: entrada escalonada dos cards, brilho nos botões, pulso nas ofertas, respeito a `prefers-reduced-motion`
- Contador de ferramentas dinâmico (ajustado ao catálogo real)
